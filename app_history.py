...
def get_openweather_history_daily(
    ...
    """Retrieve daily historical records, following OpenWeather pagination."""
    ...
    records = [collected[key] for key in sorted(collected.keys(), reverse=True)[:days]]
    records.reverse()
    return {"timezone": timezone_name, "data": records}


def normalize_temperature(value: Any) -> float | None:
    """Return Fahrenheit, correcting records returned in Kelvin."""
    try:
        number = float(value)
...
def normalize_wind(value: Any, kelvin_record: bool) -> float | None:
    """Return mph; standard-unit pages report wind in metres per second."""
    try:
        number = float(value)
    except (TypeError, ValueError):
        return None
    return number * 2.236936 if kelvin_record else number
...
def rain_inches(record: dict[str, Any]) -> float:
    ...
    try:
        value = float(rain or 0)
    except (TypeError, ValueError):
        return 0.0

    # Historical timeline responses commonly return precipitation in mm.
    return value / 25.4 if value > 0 else 0.0


def history_description(record: dict[str, Any]) -> str:
    description = weather_description(record)
    if description != "Unknown":
        return description
    for key in ("summary", "description", "weather_summary"):
...
def history_dataframe(payload: dict[str, Any]) -> pd.DataFrame:
    ...
    for item in payload.get("data", []):
        ...
        raw_reference = temp.get("day", temp.get("max"))
        try:
            kelvin_record = float(raw_reference) > 170
        except (TypeError, ValueError):
            kelvin_record = False
        ...
        rows.append(
            {
                ...
                .tz_convert(tz_name)
                .date(),
                "High °F": normalize_temperature(temp.get("max")),
                "Low °F": normalize_temperature(temp.get("min")),
                "Average °F": normalize_temperature(temp.get("day")),
                "Rain in": rain_inches(item),
                "Humidity %": item.get("humidity"),
                "Pressure hPa": item.get("pressure"),
                "Wind mph": normalize_wind(item.get("wind_speed"), kelvin_record),
                "Gust mph": normalize_wind(item.get("wind_gust"), kelvin_record),
                "Conditions": history_description(item),
            }
        )
...
if page == "Home":
    ...
    st.caption(
        "OpenWeather daily history for Gillsburg. "
    ...
    try:
        ...
        if history_df.empty:
            ...
            m1.metric("Highest temperature", f"{history_df['High °F'].max():.1f} °F")
            m2.metric("Lowest temperature", f"{history_df['Low °F'].min():.1f} °F")
            m3.metric("Total precipitation", f"{history_df['Rain in'].sum():.2f} in")
            m4.metric("Average humidity", f"{history_df['Humidity %'].mean():.0f}%")
            ...
            st.dataframe(
                history_df.style.format(numeric_formats, na_rep="—"),
